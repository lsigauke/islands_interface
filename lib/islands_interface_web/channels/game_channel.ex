defmodule IslandsInterfaceWeb.GameChannel do
  ...
  alias IslandsEngine.{Island, Game, GameSupervisor}

  def join("game:" <> _player, %{"screen_name" => screen_name}, socket) do
    if authorised?(socket, screen_name) do
      Island.types()
      send(self(), {:after_join, screen_name})
      {:ok, socket}
    else
      {:error, %{reason: "Uauthorised!"}}
    end
  end
  ...
  def handle_in("guess_coordinate", params, socket) do
    ...
    case Game.guess_coordinate(via(socket.topic), player, row, col) do
      {:hit, island, win} ->
        ...
        broadcast!(socket, "player_guessed_coordinate", Map.put(params, :result, result))
        {:noreply, socket}
  ...
  defp authorised?(socket, screen_name) do
